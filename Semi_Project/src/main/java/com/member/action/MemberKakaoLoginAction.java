...
public class MemberKakaoLoginAction implements Action {
	...
	String kakao_id = request.getParameter("member_id").trim();
	...
	String kakao_email = request.getParameter("member_email").trim();
		...
		MemberDTO memdto = dao.KakaoLogin(kakao_id, kakao_name, kakao_email);
		...
		if (memdto != null) {
			...
			if (memdto.getMember_type() == 0) {
				...
				out.println("<script>");
				out.println("alert('" + session.getAttribute("name") + " 카카오 회원님 다시 오신걸 환영합니다.')");
				out.println("location.href='main.jsp'");
				out.println("</script>");
