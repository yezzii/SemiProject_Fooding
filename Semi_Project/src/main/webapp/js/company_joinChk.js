...
function checkAll() {
	...
	if ($("#reg-name").val() == "") {
		...
		$("#reg-namechk").text(""); //span 태그 영역 초기화.
		$("#reg-namechk").show();
		$("#reg-namechk").append(warningTxt);
		$("#reg-name").val('').focus();//input 태그 영역 초기화.		

		return false;
	}
	if (!nameRegExp.test($("#reg-name").val())) {
	...
	if ($("#reg-email").val() == "") {
		...
		$("#reg-emailchk").text(""); //span 태그 영역 초기화.
		$("#reg-emailchk").show();
		$("#reg-emailchk").append(warningTxt);
		$("#reg-email").val('').focus();//input 태그 영역 초기화.		
		
		return false;
	}
	if (!emailRegExp.test($("#reg-email").val())) {
